...
CREATE TABLE [dm].[FACT_PARTICIPATION_FP] (
    id INT IDENTITY(1,1) PRIMARY KEY,

    -- Clés étrangères vers les dimensions (nullable)
    INSCRIPTION_SK      INT NULL,
    BENEFICIAIRE_SK     INT NULL,
    ETABLISSEMENT_SK    INT NULL,
    CANDIDAT_SK         INT NULL,
    DATE_SK_candidat    INT NULL,
    DATE_SK_detenu      INT NULL,
    DATE_SK_annee       INT NULL,

    -- Contraintes de clés étrangères avec ON DELETE CASCADE
    CONSTRAINT FK_FACT_FP_INSCRIPTION      FOREIGN KEY (INSCRIPTION_SK)     REFERENCES [dm].[DIM_INSCRIPTION](INSCRIPTION_SK)     ON DELETE CASCADE,
    CONSTRAINT FK_FACT_FP_BENEFICIAIRE     FOREIGN KEY (BENEFICIAIRE_SK)    REFERENCES [dm].[DIM_BENEFICIAIRE](BENEFICIAIRE_SK)    ON DELETE CASCADE,
    CONSTRAINT FK_FACT_FP_ETABLISSEMENT    FOREIGN KEY (ETABLISSEMENT_SK)   REFERENCES [dm].[DIM_ETABLISSEMENT](ETABLISSEMENT_SK) ON DELETE CASCADE,
    CONSTRAINT FK_FACT_FP_CANDIDAT         FOREIGN KEY (CANDIDAT_SK)        REFERENCES [dm].[DIM_CANDIDAT](CANDIDAT_SK)           ON DELETE CASCADE,

    -- Contraintes de clés étrangères explicites sans cascade
    CONSTRAINT FK_FACT_FP_DATE_CANDIDAT    FOREIGN KEY (DATE_SK_candidat)   REFERENCES [dm].[DIM_DATE](DATE_SK) ON DELETE NO ACTION,
    CONSTRAINT FK_FACT_FP_DATE_DETENU      FOREIGN KEY (DATE_SK_detenu)     REFERENCES [dm].[DIM_DATE](DATE_SK) ON DELETE NO ACTION,
    CONSTRAINT FK_FACT_FP_DATE_ANNEE       FOREIGN KEY (DATE_SK_annee)      REFERENCES [dm].[DIM_DATE](DATE_SK) ON DELETE NO ACTION
);




CREATE TABLE dm.FACT_offre_de_formation (
    FACT_ID INT IDENTITY(1,1) PRIMARY KEY,
    ...
    -- 🔗 Clés étrangères renommées
    CONSTRAINT FK_FACT_OFFRE_ETABLISSEMENT FOREIGN KEY (ETABLISSEMENT_SK)
        REFERENCES dm.DIM_ETABLISSEMENT(ETABLISSEMENT_SK) ON DELETE CASCADE,

    CONSTRAINT FK_FACT_OFFRE_FILIERE FOREIGN KEY (FILIERE_SK)
        REFERENCES dm.DIM_FILIERE(FILIERE_SK) ON DELETE CASCADE,

    CONSTRAINT FK_FACT_OFFRE_ETAB_HOMO FOREIGN KEY (ETAB_HOMOLOGATION_SK)
        REFERENCES dm.DIM_ETABLISSEMENT_HOMOLOGATION(ETAB_HOMOLOGATION_SK) ON DELETE CASCADE,

    CONSTRAINT FK_FACT_OFFRE_ETAB_FILIERE FOREIGN KEY (ETAB_FILIERE_SK)
        REFERENCES dm.DIM_ETABLISSEMENT_FILIERE(ETAB_FILIERE_SK) ON DELETE CASCADE,

    CONSTRAINT FK_FACT_OFFRE_PROVINCE FOREIGN KEY (PROVINCE_SK)
        REFERENCES dm.DIM_PROVINCE(PROVINCE_SK) ON DELETE CASCADE,

    CONSTRAINT FK_FACT_OFFRE_DATE_ETAB FOREIGN KEY (date_etab_KEY)
        REFERENCES dm.DIM_DATE(DATE_SK) ON DELETE NO ACTION,

    CONSTRAINT FK_FACT_OFFRE_DATE_CLOTURE FOREIGN KEY (DateCloture_key)
        REFERENCES dm.DIM_DATE(DATE_SK) ON DELETE NO ACTION,

    CONSTRAINT FK_FACT_OFFRE_DATE_AUTORISATION FOREIGN KEY (DATE_SK_AUTORISATION)
        REFERENCES dm.DIM_DATE(DATE_SK) ON DELETE NO ACTION
);

ALTER TABLE [dm].[FACT_offre_de_formation]
ADD programme_sk INT,
    date_val_sk INT,
    date_depot_sk INT;

ALTER TABLE [dm].[FACT_offre_de_formation]
ADD CONSTRAINT FK_FACT_offre_de_formation_Programme
    FOREIGN KEY (programme_sk) REFERENCES [dm].[DIM_Programme_validation_request]([programme_sk]);

ALTER TABLE [dm].[FACT_offre_de_formation]
ADD CONSTRAINT FK_FACT_offre_de_formation_DateVal
    FOREIGN KEY (date_val_sk) REFERENCES [dm].[DIM_DATE]([DATE_SK]);

ALTER TABLE [dm].[FACT_offre_de_formation]
ADD CONSTRAINT FK_FACT_offre_de_formation_DateDepot
    FOREIGN KEY (date_depot_sk) REFERENCES [dm].[DIM_DATE]([DATE_SK]);
